// This C program simulates protocol data unit (PDU) encapsulation across the Transport, Network, and Data Link layers, concluding with a demonstration of synchronous bit framing and zero-bit stuffing for data transparency.
...
void senderBit(unsigned char *data, int len) {
    ...
    // 1. Framing Start Flag
    fprintf(fp, "01111110");
    printf("-> Emitting Start Flag          : 01111110\n");

    // 2. Control Header Segment
    fprintf(fp, "10101010");
...
void receiverBit() {
    ...
    int sIdx = 16;          // Skip Start Flag + Header
    int eIdx = len - 16; // Ignore Trailer + End Flag
